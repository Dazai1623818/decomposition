...
final class CPQPathInspectionTest {
    ...
    void inspectCycleCandidate() {
        ...
        graph.getVertices().forEach(v ->
                System.out.println("  " + v.getID() + " -> " + graph.getVertexLabel(v)));
        ...
        unique.getEdges().forEach(edge ->
                System.out.println("  " + edge.getSourceNode().getData().getName()
    ...
    void inspectBacktrackVariants() {
        ...
        for (int i = 0; i < expressions.length; i++) {
            ...
            graph.getVertices().forEach(v ->
                    System.out.println("  vertex " + v.getID() + " label=" + graph.getVertexLabel(v)));
            graph.getEdges().forEach(e ->
