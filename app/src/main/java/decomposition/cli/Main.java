 ...
 *
 * <p>Runs both decomposition modes on a built-in example CQ and prints the resulting CPQs.
 *
 * <p>Usage:
 *
 * <ul>
...
public final class Main {
  ...
  public static void main(String[] args) {
    ...
    List<List<CPQExpression>> exhaustive =
  ...
  private static void printDecompositions(List<List<CPQExpression>> decompositions) {
    for (int i = 0; i < decompositions.size(); i++) {
      List<CPQExpression> expressions = decompositions.get(i);
      System.out.println("  Decomposition " + (i + 1) + " (" + expressions.size() + " CPQs):");
      expressions.forEach(e -> System.out.println("    - " + e.cpq()));
    }
  }
}
