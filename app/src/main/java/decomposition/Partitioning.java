...
public class Partitioning {
    ...
    public static List<List<Set<Edge>>> enumerateConnectedEdgePartitions(CQ cq) {
        List<Edge> edges = QueryUtils.extractEdgesFromCQ(cq);
        List<List<Set<Edge>>> result = new ArrayList<>();
        findPartitions(edges, new ArrayList<>(), new HashSet<>(), result);
        result.sort(partitionOrderComparator()); // sort here
        return result;
    }

    private static void findPartitions(List<Edge> remaining, List<Set<Edge>> current, Set<String> memo, List<List<Set<Edge>>> result) {
        if (remaining.isEmpty()) {
            String key = canonicalPartition(current);
            if (memo.add(key)) {
                List<Set<Edge>> snapshot = current.stream()
                        .map(HashSet::new)
        ...
        for (Set<Edge> subset : generateConnectedSubsets(remaining)) {
    ...
    public static List<Set<Edge>> generateConnectedSubsets(List<Edge> edges) {
        List<Set<Edge>> subsets = new ArrayList<>();
        int n = edges.size();
    ...
    public static boolean isConnected(Collection<Edge> edges) {
        ...
        stack.push(iterator.next().source);
    ...
    public static List<Edge> listSubtract(List<Edge> a, Collection<Edge> b) {
        List<Edge> result = new ArrayList<>(a);
        for (Edge e : b) result.remove(e); // remove once, preserving multiplicity
        return result;
    }

    public static String canonicalPartition(List<Set<Edge>> partition) {
    ...
    public static Set<String> getVertices(Collection<Edge> edges) {
        return edges.stream()
                .flatMap(e -> Stream.of(e.source, e.target))
                .collect(Collectors.toSet());
    }

    public static List<Set<String>> getJoinNodesPerComponent(List<Set<Edge>> partition, Set<String> freeVariables) {
    ...
    public static List<List<Set<Edge>>> filterPartitionsByJoinConstraint(
            List<List<Set<Edge>>> partitions,
    ...
    public static Comparator<List<Set<Edge>>> partitionOrderComparator() {
        return Comparator
                .comparing((List<Set<Edge>> p) ->
                        p.stream().mapToInt(Set::size).max().orElse(0)) // max component size
                .thenComparing(p ->
                        Collections.frequency(
                                p.stream().map(Set::size).toList(),
                                p.stream().mapToInt(Set::size).max().orElse(0))) // freq of max
