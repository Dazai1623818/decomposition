...
/**
 * Generates:
 *  1) All non-empty, connected, edge-induced subgraphs ("components")
 *     of an input CQ edge list; and
 *  2) All edge-disjoint partitions of the entire CQ into such components.
 *
 * Notes on terminology:
 *  - "Component" here means *any* connected subset of edges (not only maximal).
 *  - "Partition" is a list of components whose edge sets are disjoint and
 *    whose union covers all edges of the CQ.
 *
 * Runtime:
 *  - enumerateConnectedComponents explores the lattice of connected edge-sets
 *    (exponential in the worst case, pruned by connectivity).
 *  - enumeratePartitions is a backtracking set-partition over those components
 *    (also exponential in the worst case).
 */
public final class PartitionGenerator {
     ...
     *
     * Strategy:
     *  - Seed with each single edge (BitSet with one bit set).
     *  - Recursively "expand" by adding any edge touching the current
     *    vertex frontier (so we remain connected).
     *  - Use a global "seen" set (by BitSet signature) to avoid duplicates
     *    that would arise from different seeds / orders.
     */
    ...
    /**
     * Enumerate all partitions of the full CQ edge set into edge-disjoint components
     * (i.e., a cover by components with no overlap).
     *
    ...
    /**
     * Compute the set of edges that can be added while preserving connectivity:
     * any edge with at least one endpoint in the current vertex frontier.
     */
    ...
    /**
     * Build an inverted index: for each edge index i, list all components that contain i.
     * Used to quickly find candidate components that can cover the next uncovered edge.
     */
    ...
    /**
     * Backtracking search to build all edge-disjoint covers:
     *  - 'used' marks currently covered edges,
     *  - choose the smallest-index uncovered edge,
     *  - try each component that includes it and doesn't overlap 'used',
     *  - stop when we've covered 'allEdges'.
     */
